...
; This example drives SOUND from the match: a text query -> nearest embedding ->
; an additive sine bank (each coordinate = amplitude of a partial at a fixed
; center frequency), and a spoken query (STT) -> filter impulse response -> ftconv.
; The query's MEANING shapes the timbre.
; -----------------------------------------------------------------------------
...
instr OSCS
    ...
    neighs:k[][], scores:k[], kgate:k = semspacequerytxt(s_handle, e_handle, "warm analog texture", 3)
